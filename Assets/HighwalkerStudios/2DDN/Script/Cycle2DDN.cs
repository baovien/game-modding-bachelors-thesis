...
public class Cycle2DDN : MonoBehaviour {
    ...
    public float spawnValuesXStart;
    public float spawnValuesXEnd;
    public float spawnValuesYStart;
    public float spawnValuesYEnd;
	...
	void Update () {
        ...
        if(cycle == 1 && haveSpawned == false)
        {
            for (int i = 1; i <= numberOfZombies; i++)
            {
                Vector3 spawnLocation = new Vector3(Random.Range(spawnValuesXStart, spawnValuesXEnd), Random.Range(spawnValuesYStart, spawnValuesYEnd), 0);
                Quaternion spawnRotation = Quaternion.identity;
                Instantiate(zombie, spawnLocation, spawnRotation);
                haveSpawned = true;
            }
        }
	}
